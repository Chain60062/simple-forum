...
export const addTopic = async (req, res, next) => {
    try {
        const sqlQuery = 'INSERT INTO topic(topic_name, description) VALUES($1, $2) RETURNING topic_name, description, topic_id';
...
export const listTopics = async (req, res, next) => {
    try {
        const topics = await pool.query('SELECT topic_id, topic_name, description FROM topic ORDER BY topic_name');
