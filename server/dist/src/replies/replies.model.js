...
export const addReply = async (req, res, next) => {
    try {
        ...
        const replyQuery = `WITH inserted AS (INSERT INTO reply(profile_id, post_id, parent_id, message) VALUES($1, $2, $3, $4) RETURNING *) SELECT * FROM 
...
export const getPostReplies = async (req, res, next) => {
    try {
        const postId = req.params.postId;
        const replies = await pool.query(`with recursive replies as (select r.reply_id, r.message, r.parent_id, r.post_id, r.created_at, p.nickname, p.profile_name, p.avatar from reply r join profile p on p.profile_id = r.profile_id 
      ...
      join replies on child.parent_id = replies.reply_id join profile p on p.profile_id = child.profile_id) select * from replies r order by r.created_at desc`, [postId]);
