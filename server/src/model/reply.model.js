...
export const addReply = async (req, res, next) => {
  try {
    ...
    Promise.all(
      ...
      .then(res.status(200).json({ message: reply.rows[0].message }))
      .catch(next);
  } catch (err) {
    next(err);
  }
  ...
  try {
    ...
    await pool
      ...
      .then(() => {
        ...
        jsonMessage(res, 200, 'Comentário deletado com sucesso');
      })
      .catch(next);
  } catch (err) {
    next(err);
...
from
	reply child
join replies on child.parent_id = replies.reply_id
)
select
	r.reply_id,
	r.post_id,
