...
export const createUser = async (req, res, next) => {
  try {
    const { name, password, nickname, email } = req.body; //message body
    const avatar = req.file; //profile picture
    const sqlQuery =
    ...
    await pool.query(sqlQuery, [hash, avatar.path, name, 'user', false, nickname, email]);
    jsonMessage(res, 200, 'Usuário criado com sucesso');
  } catch (err) {
    // object literal sql error check
    next(err);
    // (sqlErrors[err.constraint] || sqlErrors['default'])(res);
  }
};

export const editUser = async (req, res, next) => {
  try {
    const userId = 'acd3fee5-6e10-4a6f-9af4-419700bcf5cf'; //req.session.user.user_id;
...
export const deleteUser = async (req, res, next) => {
  try {
    ...
    // delete profile picture from public folder, but not if its the default one.
