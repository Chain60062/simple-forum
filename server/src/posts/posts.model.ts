...
export const deletePost = async (
	...
	try {
		...
		if (await removePostFilesFromDatabase(postId, userId, files))
			return res.status(500).json('Erro interno ao remover imagens do post')

		if (files === null) return res.status(404).json('Post não encontrado')
	...
	async function removePostFilesFromDrive(files: QueryResult) {
